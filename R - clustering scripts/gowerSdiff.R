...
clusterdata <- read.csv(file="comp_df_diff.csv", header=TRUE, sep=",")
...
write.csv(similarity, file = "Structsimilar_diff.csv", row.names = TRUE)
...
medoids
write.csv(medoids, file = "medoidscomplete_diff.csv", row.names = TRUE)


clusteroutput <- cbind(clusterdata,pam_fit$clustering)
write.csv(clusteroutput, file = "PAMcomplete_diff.csv", row.names = TRUE)